...
# Import only DeepgramClient to avoid ImportError
...
if DEEPGRAM_API_KEY:
    try:
        # Use api_key as a keyword argument
...
async def transcribe_audio_deepgram(
    ...
    try:
        ...
        # 3. Prepare transcription options using a standard Python dictionary
        # This will be passed as keyword arguments to the SDK method.
        options = {
            "model": "nova-3",
        ...
        # FIX: We pass the file path as the single positional argument 
        # and the options dictionary as keyword arguments (kwargs).
        response = await asyncio.to_thread(
            deepgram_client.listen.v1.media.transcribe_file,
            compressed_path, # Positional Argument 1: File Path
            **options          # FIX: Options are unpacked as keyword arguments 
        )
